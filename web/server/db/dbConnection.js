...
db.consultaMortalidad = function(done){

    var query = "select i.nombre as 'indicador', i.fuente as 'fuente' ,p.nombre as 'letter',s.nombre as sexo ,d.value as 'frequency' ,a.nombre as 'date' from datos d " +
        "inner join indicadores i on i.id = d.id_indicadores " +
        "inner join provincias p on p.id = d.id_provincia " +
        "inner join anios a on d.id_anio = a.id " +
        "inner join sexo s on s.id = d.id_sexo " +
        "where i.nombre = 'Mortalidad intrahospitalaria global por cada 100 altas hospitalarias' " +
        "and a.id > 19 and s.nombre='total' and a.nombre=2013" ;
...
db.consultaEspecialistas = function(done){

    var query = "select i.nombre as 'indicador', i.fuente as 'fuente' ,p.nombre as 'provincia',s.nombre as sexo ,d.value as 'frequency' ,a.nombre as 'date' from datos d" +
        " inner join indicadores i on i.id = d.id_indicadores" +
        " inner join provincias p on p.id = d.id_provincia" +
        " inner join anios a on d.id_anio = a.id " +
        " inner join sexo s on s.id = d.id_sexo " +
        " where i.nombre = 'Personal médico en atención especializada por 1000 hab.' " +
        " and a.id > 19 and s.nombre='total' and a.nombre=2013" ;
